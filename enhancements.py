...
class EnhancedApp(base.App):
    """v4.1 integration layer: global drag/drop, live preview hooks, source-folder export."""

    def __init__(self):
        # Must exist before base __init__, because base loads saved configuration dynamically.
        self.output_to_source = tk.BooleanVar(value=False)
        self.source_subfolder = tk.StringVar(value="加工後")
        super().__init__()
    ...
    def on_drop(self, event):
        """Accept one/many images and folders from anywhere in the app window."""
        try:
        ...
        for raw in items:
            ...
            if p.is_file() and p.suffix.lower() in base.SUPPORTED:
                ...
                # Recursive folder drop is convenient for real production folders.
                try:
                    found.extend(str(x) for x in p.rglob("*") if x.is_file() and x.suffix.lower() in base.SUPPORTED)
                except Exception:
                    pass
    ...
    def _install_source_export_bar(self):
        ...
        entry = ttk.Entry(bar, textvariable=self.source_subfolder, width=16)
    ...
    def _refresh_enhanced_summary(self):
        try:
            ...
            if self.output_to_source.get():
                dest = f"原圖資料夾 / {self._safe_subfolder()}"
            else:
                dest = "自訂輸出資料夾"
            self.export_summary.set(
                f"輸出：{self.output_ratio.get()} · {size_text} · "
                f"{self.output_format.get()} · 品質 {self.quality.get()} · {dest}"
            )
        except Exception:
            pass
    ...
    def finish(self, done, skip, errors, out):
        ...
        if self.output_to_source.get():
            destination = f"各原圖資料夾內的『{self._safe_subfolder()}』"
        else:
            destination = str(out)
